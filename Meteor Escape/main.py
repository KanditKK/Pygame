...
entities = [] #[px, py ,type, xc, yc, subtype]
bullet = [] #[px, py]
...
while not start:
    ...
    text = bigger_font.render('Meteor Escape!', True, (255,255,255))
    textRect = text.get_rect()
    textRect.center = (display_width // 2, (display_height // 2)-25)
    gameDisplay.blit(text, textRect)

    text = smaller_font.render('Use A,D to control, W to boost!', True, (255,255,255))
    textRect = text.get_rect()
...
while not crashed:
    ...
    ##
    x += x_change
    if x <= 0: crashed = renderPauseScreen('dead', getMessage('dead', 'border')); x = (display_width * 0.45); x_change = 0; entities = []; score = 0; gameSpeed = 1
    if x >= display_width - 100: crashed = renderPauseScreen('dead', getMessage('dead', 'border')); x = (display_width * 0.45); x_change = 0; entities = []; score = 0; gameSpeed = 1
    if score < 50:
    ...
    if checkHitbox(x, entities): crashed = renderPauseScreen('dead', getMessage('dead', 'hit-by-meteor')); x = (display_width * 0.45); x_change = 0; entities = []; score = 0; gameSpeed = 1
    maxtxt = open('score.txt', 'r')
    max = int(str(maxtxt.readline()))
    maxtxt.close()
